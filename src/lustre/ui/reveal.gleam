...
//
pub const name: String = "lustre-ui-reveal"
...
//
pub fn reveal(attributes: List(Attribute(msg))) -> Element(msg) {
...
//
pub fn value(message: String) -> Attribute(msg) {
...
pub fn easing_function(function: tween.Function) -> Attribute(msg) {
  attribute("easing-function", tween.to_string(function))
}
...
fn update(model: Model, msg: Msg) -> #(Model, Effect(Msg)) {
  case msg {
    ...
    ParentSetDuration(duration) -> {
      let model =
        Model(..model, duration: duration, target: model.start +. duration)
      let effect = effect.none()
    ...
    ParentSetTarget(message, start) if model.duration <. 1.0 -> {
      let to = string.length(message) |> int.to_float
      let model =
        Model(
          ..model,
          message:,
...
fn view(model: Model) -> Element(Msg) {
  ...
  element(
    ...
    [
      value
      |> int.min(target)
      |> string.slice(model.message, 0, _)
      |> html.text,
    ],
  )
}
